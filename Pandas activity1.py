...
], columns=["Name", "Average Temperature (°C)", "Radius", "Colour", "Interesting Feature"])
...
planet_selected = input("Which planet would you like to learn about?    > ").capitalize()

if planet_selected in df.index:
    ...
    if info_selected=="All": print(df.loc[f'{planet_selected}'])

    elif info_selected in df.columns:
        print(f"\n{info_selected} for {planet_selected}: {df.loc[planet_selected, info_selected]}")
    else:
